	...
	return (
		<ProtectedRoute>
			<PageTransition>
				<div
					ref={scrollToTopRef}
					className='flex flex-col items-center min-h-screen mt-0 md:mt-6'>
					<div
					...
					<div className='w-full max-w-7xl mx-auto px-6 md:px-0 mt-4 md:mt-6'>
						<div className='flex md:flex-row items-center justify-between w-full gap-4'>
								...
								<AnimatePresence>
									{dropdownOpen && (
										<motion.div
											...
											exit={{ opacity: 0, y: -10 }}
											transition={{ duration: 0.2 }}
											className='absolute top-full mt-0.5 right-0 rounded-lg overflow-hidden z-50 w-30'
											style={{
												backgroundColor:
													'var(--color-bg-card)',
											}}>
					...
					{filtered.length === 0 ? (
						...
						<div className='flex flex-col items-center'>
							<div className='grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 py-6 max-w-7xl mx-auto px-6 md:px-0'>
								{paginatedFiltered.map((entry, index) => (
									<motion.div
										key={entry.anime_id}
